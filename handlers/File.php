...
class File extends BaseHandler
{
    ...
    protected function createFileResource($file)
    {
        ...
        if (($dirSlash = strpos($file, '/') !== false) || (strpos($file, '\\') !== false)) {
            ...
            if (!file_exists($dir) && !mkdir($dir, $this->directoryMode, true)) {
                throw new Exception("Unable to create log directory '{$dir}'");
            }
        }
    ...
    {
        if ((int)$size <= 0) {
            throw new InvalidConfigException("File size must be greater than 0");
        }
    ...
    {
        if ((int)$count <= 0) {
            throw new InvalidConfigException("Files count must be greater than 0");
        }
